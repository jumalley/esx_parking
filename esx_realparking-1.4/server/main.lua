	...
	FindPlayerVehicles(xPlayer.identifier, function(vehicles)
		...
		if GetVehicleNumOfParking(vehicleData.parking) > Config.ParkingLocations[vehicleData.parking].maxcar then
			cb({
				...
				exports['okokNotify']:Alert("Parking", "Ce parking est plein, vous ne pouvez pas vous garer ici.", 5000, 'error'),
			})
		elseif isFound then
			MySQL.Async.fetchAll("SELECT * FROM car_parking WHERE owner = @identifier AND plate = @plate", {
				...
				if type(rs) == 'table' and #rs > 0 then
					cb({
						...
						exports['okokNotify']:Alert("Parking", "Ce parking a déjà stocké une voiture avec la même plaque.", 5000, 'error'),
					})
				else
			...
			cb({
				...
				exports['okokNotify']:Alert("Parking", "Vous devez posséder la voiture pour la garer.", 5000, 'error'),
			})
		end
	end)
end)
				...
				if type(rs) == 'table' and #rs > 0 and rs[1] ~= nil then
					...
					if playerMoney >= fee then
						...
						cb({
							...
							exports['okokNotify']:Alert("Parking", "Vous n'avez pas assez d'argent.", 5000, 'error'),
						})
					end
				else
					cb({
						...
						--message = _U("invalid_car"),
							exports['okokNotify']:Alert("Parking", "Vous n'avez pas entreposé la voiture ici ou vous n'avez pas l'autorisation de conduire cette voiture.", 5000, 'error'),
					})
				end
			end)
		else
